...
void ParticleSystem::Update(float delta)
{
	...
	for (auto it = m_moduleList.begin(); it != m_moduleList.end(); ++it)
	{
		...
		//	std::cout << "SpawnModule executed" << std::endl;
		//}
		//else if (auto* lifeModule = dynamic_cast<LifeModuleCS*>(&module)) {
		//	std::cout << "LifeModule executed" << std::endl;
		//}
#endif
	}
	...
	if (++frameCount % 60 == 0) { // 1초마다 출력
		UINT inactiveCount = ReadInactiveParticleCount(); // 이 메서드도 추가 필요
		std::cout << "Active particles: " << m_activeParticleCount
			<< ", Inactive particles: " << inactiveCount << std::endl;
	}
#endif
}
